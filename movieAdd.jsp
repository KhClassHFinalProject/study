...
<head>
	...
	<script type="text/javascript" src="/myweb2/js/jquery-3.2.1.min.js"></script>
			...
			function() {
				...
				$("#tableButton").on("click",
					function() {
						...
						var numVal=$("#tableNum").val();
						$("#tableNum").prop("disabled", true);
				...
				$("table").on("click",
					function() {
						...
						var numVal=$("#tableNum").val(); */
						var rowName=event.target.className;
						var colName=parseInt($(event.target).text());
						var colVal=parseInt($("#tableCol").val());
						var numVal=parseInt($("#tableNum").val());
						var colnum=parseInt(colName)+parseInt(numVal);
						...
						if((colnum)<=colVal){
							for(var i=colName;i<(colnum);i++){
								$("."+rowName+":eq("+i+")").css("background-color", "#00ffff");
							}
						}else{
							var subtcn = colnum-colVal;
							colName-=subtcn;
							
							for(var i=colName;i<(colName+numVal);i++){
									$("."+rowName+":eq("+i+")").css("background-color", "#00ffff");
							}
						}
						alert("선택자리 : " + rowName + (colName+subtcn));
						$(".step").eq(1).prop("checked", true);
						$(".step").eq(2).prop("checked", true);
					}
				);
...
<body>
	자리<select name="tableNum" id="tableNum"></select>
