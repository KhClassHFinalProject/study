...
function joinClick(){
	if($('#id').val()==""){
		...
		alert('중복검사를 해주세요');
	}else if($('#idCheck').val()!=$('#id').val()){
		alert('다시 입력한 아이디를 중복검사 해주세요.');
	}else if($('#pwd').val()==""){
		alert('비밀번호를 입력해 주세요!')
	}else if($('#pwd').val()!=$("#pwdCheck").val()){
		alert('비밀번호를 확인해 주세요!')
	}else if($('#textCheck').val() != "<%=autoJoin%>"){
		alert('자동가입방지 문자를 확인해주세요!')
	}else{
		alert('가입 승인!');
		$("#joinData").html("아이디 : " + $('#id').val() + "\n비밀번호 : " + $("#pwd").val());
	} 
	console.log($("#id").val());
...
function multiCheck(){
	if($("#id").val()==""){
		...
		open("idCheck.jsp?id="+$("#id").val()+"","idCheck","width=400, height=200");
	}
