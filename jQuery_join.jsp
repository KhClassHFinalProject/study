		...
		function unlock() {
			$('#clause').on('scroll', function(){
				...
				if(myScrollbarPosition==endPosition){
					var onOff = $(":radio, #agreeRadio");
					for(var i=0 ; i<onOff.length ; i++){
						onOff[i].disabled = false;
					}
				}
			})
...
// 			var pwd2=document.getElementById("userpwdcheck").value;
			if(pwd1!=""){
				if(pwd2==""){
					$("#userpwdcheck").prop('readOnly', false).css('background-color', '#ffffff');
					$("#pwdResult").val("PWD체크를 입력하세요").css('color', 'green');
					$("#pwdHidden").val("false");
		...
		<table>
			...
			<tr>
				<td>
					...
					<input type="radio" id="agreeRadio" name="agreeRadio" disabled="disabled" onClick="clauseCheck(1)">동의
					<input type="radio" id="agreeRadio" name="agreeRadio" disabled="disabled" checked="checked" onClick="clauseCheck(2)">동의 안함
				</td>
			</tr>
